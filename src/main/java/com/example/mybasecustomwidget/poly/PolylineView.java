...
//		linePaint.setPathEffect(new CornerPathEffect(200));
		...
		for (float y = viewSize * BOTTOM - spaceY; y > viewSize * TOP; y -= spaceY) {
			for (float x = viewSize * LEFT; x < viewSize * RIGHT; x += spaceX) {
				...
				if (y == viewSize * BOTTOM - spaceY) {
					canvas.drawText(String.valueOf(rulerX[index_y]), x, y
	...
	private void drawSign(Canvas canvas) {
		...
		canvas.drawText(signY == null ? "y" : signY, textY_X, textY_Y,
				mTextPaint);

		// 绘制横轴标识文字
		mTextPaint.setTextAlign(Paint.Align.RIGHT);
		canvas.drawText(signX == null ? "x" : signX, textX_X, textX_Y,
				mTextPaint);
	...
	public synchronized void setData(List<PointF> pointFs,String signX,String singY){
		 /* 
         * 数据为空直接GG 
         */
